 ...
 *
 * Lane numbers are 1-5 (matching the drum pads). A `null` lane = a beat with
 * no note (just a backing kick, no drum requirement).
 *
 * Chart entries: { beat: <number>, lane: <1-5 | null> }
...
const SONGS = [
    {
        ...
        // Easy: mostly center lane, predictable
        chart: [
            { beat: 0,  lane: 3 },
            { beat: 2,  lane: 3 },
            { beat: 4,  lane: 1 },
    ...
    },
    {
        ...
        chart: (() => {
            // Generate procedurally for length — random but deterministic
            const c = [];
            ...
            for (let b = 0; b < 96; b += 0.5) {
                ...
                // Occasional double (same beat, second lane)
                if (rng() < 0.06) {
                    c.push({ beat: b, lane: lanes[Math.floor(rng() * 5)] });
                }
            }
